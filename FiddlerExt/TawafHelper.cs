    ...
    public class TawafHelper
    {
        ...
        public Dictionary<string,string> GetTawafConfig(Session[] oSessions)
        {
            ...
            foreach (var sess in oSessions)
            {
                ...
                if (Regex.IsMatch(sess.fullUrl,@"tawaf\.com\.sa/tawaf\d{2}/etawaf/rep_mofa", RegexOptions.IgnoreCase))
                {
                    ...
                    var mtch = Regex.Match(sess.GetRequestBodyAsString(), @"tawaf.com.sa/tawaf\d{2}/etawaf/\|(.*?)\|com.gwt.umra.client.async.Rep_Mofa_Service\|getListData\|(.*?)\|(.*?)\|(.*)\|Group Code\|,");
                    if (mtch.Success)
                    {
                ...
                else if (Regex.IsMatch(sess.fullUrl,@".tawaf.com.sa/tawaf\d{2}/etawaf/V_Mut_Groups", RegexOptions.IgnoreCase))
                {
                    ...
                    //7|0|6|http://www.tawaf.com.sa/tawaf39/etawaf/|0E955BD11DE0B65567E272F50C88C8E9|com.gwt.umra.client.async.V_Mut_Groups_Service|getTableData|com.gwt.umra.shared.wrapper.V_Mut_Groups_Wrapper/1672639591|from V_Mut_Groups where 1=1 AND grp_status=10 AND uo_code = 165  AND uo_code=165 AND uo_branch_code=0 AND ea_code=7147 AND grp_arch=0|1|2|3|4|1|5|5|0|0|0|0|0|0|20|6|0|0|0|0|0|
                     ...
                     var mtch = Regex.Match(sess.GetRequestBodyAsString(),@".tawaf.com.sa/tawaf\d{2}/etawaf/\|(.*?)\|com.gwt.umra.client.async.V_Mut_Groups_Service\|getTableData\|(.*?)\|");
                    if (mtch.Success)
                    {
